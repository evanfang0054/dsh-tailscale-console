...
import { enhancedManifest, injectPwaHead, injectInstallBootstrap } from "./pwa.js"
...
async function apply(ctx, rawConfig) {
  ...
  // ── tailnet 身份自动登录 ──────────────────────────────────────────────────
  // tailscale serve 给 tailnet 流量注入 Tailscale-User-Login 等身份头，并剥掉外部
  // 伪造的同类头（官方文档）；dsh 仅监听 127.0.0.1，LAN 亦无法插头。因此
  // 「带身份头 = 已通过 Tailscale 身份认证」→ 免配对码直接签发会话 cookie。
  const tailscaleLoginOf = (req) => {
    const v = req.headers["tailscale-user-login"]
    return typeof v === "string" && v.trim() !== "" ? v.trim() : undefined
  }
  // dsh 连接服务：authenticatedUrl() 给出带启动令牌的 URL，设备访问一次即由
  // dsh 自己完成 cookie 签发（插件零凭据接触，规避 0.1.5 的 credentials 网关挂起）。
  const tokenUrlFor = (req, authority) => {
    if (authority === undefined) return undefined
    if (tailscaleLoginOf(req) === undefined && !isLoopbackAuthority(authority)) return undefined
    const connection = ctx.get("connection")
    if (connection === undefined) return undefined
    const origin = (isLoopbackAuthority(authority) ? "http://" : "https://") + authority
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // 变更类操作（SSH 运维 / serve 开关 / 代理改写）：仅本机回环或带 tailnet
      // 身份头（serve 注入、防伪造）的请求可调用——纯读接口不受限。
      const MUTATING = new Set(["serveSet", "proxyApply", "serverInstall", "serverRelay"])
      if (MUTATING.has(method) && !isLoopbackAuthority(requestAuthorityOf(req)) && tailscaleLoginOf(req) === undefined) {
        res.writeHead(403, { "content-type": "application/json" })
        res.end(JSON.stringify({ ok: false, error: { code: "forbidden", message: "变更操作仅限本机或带 tailnet 身份的请求" } }))
        return
      }
      let args = {}
      try {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // 令牌 URL：dsh 自己完成 cookie 签发并 303 回首页；配对码参数仅为旧链接兼容
      const tokenUrl = tokenUrlFor(req, authority)
      if (tokenUrl !== undefined) {
        res.writeHead(303, { "cache-control": "no-store", "location": tokenUrl, "referrer-policy": "no-referrer" })
        res.end()
        return
      }
      res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" })
      res.end(renderPairPage({ error: "无法确认 tailnet 身份。请从 tailnet 内的设备访问，或重启 dsh web 后在 Mac 面板重新生成链接。" }))
    },
  }), "dsh-tailscale-console: /tsctl/pair route")

  // PWA Service Worker：纯透传（不缓存不改编写），仅用于满足 Chrome 一键安装条件。
  // Service-Worker-Allowed: / 使其可注册根作用域。
  ctx.effect(() => ctx.webServer.register({
    kind: "prefix",
    path: "/tsctl/sw.js",
    handler: async (_req, res) => {
      ...
      res.end("self.addEventListener('install',function(){self.skipWaiting()});self.addEventListener('activate',function(e){e.waitUntil(self.clients.claim())});self.addEventListener('fetch',function(e){e.respondWith(fetch(e.request))})")
    },
  }), "dsh-tailscale-console: service worker route")

  // 一键安装引导：应用页面注册 SW + 捕获 beforeinstallprompt（面板按钮消费）
  ctx.effect(() => ctx.webServer.tapIndex((html) => {
    try { return injectInstallBootstrap(html) } catch { return html }
  }), "dsh-tailscale-console: install bootstrap injection")

  // 自动登录路由：已装 APP 的 start_url。tailnet 身份头或本机回环 → 静默续登 303；
  // 其余（公网误入等）→ 配对页。
  ctx.effect(() => ctx.webServer.register({
    kind: "prefix",
    path: "/tsctl/auto-login",
    handler: async (req, res) => {
      const authority = requestAuthorityOf(req)
      const tokenUrl = tokenUrlFor(req, authority)
      if (tokenUrl !== undefined) {
      ...
      res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" })
      res.end(renderPairPage({ error: "无法确认 tailnet 身份。请从 tailnet 内的设备访问。" }))
    },
  }), "dsh-tailscale-console: auto-login route")
