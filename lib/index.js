...
async function apply(ctx, rawConfig) {
  ...
  const handlers = {
    /** 设备配对：生成本机一次性配对码（分发处强制仅回环可调用）。 */
    pairingCode: async () => {
      const { code, expiresAt } = pairing.issue(Date.now())
      return { code, expiresAt, ttlMinutes: 10 }
    },
    /** 已解析配置（安全子集）——客户端据此渲染。 */
    config: async () => ({
      dshPort: cfg.dshPort,
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (authority === undefined || !pairing.redeem(code, Date.now()).ok) {
        res.writeHead(403, { "content-type": "text/plain; charset=utf-8" })
        res.end("配对码无效或已过期。请在 Mac 本机打开 dsh 面板 → Tailscale 控制台 → 设备配对，重新生成链接。")
        return
      }
      let setCookie
      try {
