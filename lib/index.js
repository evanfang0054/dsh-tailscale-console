...
async function apply(ctx, rawConfig) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (authority !== undefined && (tailscaleLoginOf(req) !== undefined || isLoopbackAuthority(authority))) {
        await mintAutoLogin(res, authority)
        return
      }
      let code = ""
      try {
      ...
      // 成功落点 = 安装门户页：一键安装按钮 + 进入控制台（cookie 由本响应 Set-Cookie 种下）
      res.writeHead(200, {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" })
      res.end(renderPairPage({ error: undefined }))
    },
