  ...
  const handlers = {
    ...
    health: async () => {
      ...
      if (!base) {
        ...
        const sl = await run(`curl -s --noproxy '*' --max-time 8 -X POST ${base}/api/session.list -H 'content-type: application/json' -d '{"type":"client-request","rpcId":"h-1","method":"session.list","payload":{}}'`, 20000)
        let slOk = false, slDetail = sl.stdout.slice(0, 60)
        try {
          const d = JSON.parse(sl.stdout)
