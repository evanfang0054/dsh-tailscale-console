  ...
  const handlers = {
    ...
    health: async () => {
      ...
      if (!base) {
        ...
        const pageCode = page.stdout.trim()
        // 401 = 入口可达且 dsh 登录围栏正常拦下无凭据探测（curl 本就不带 cookie/令牌），
        // 这本身就是链路健康的证明；真故障表现为超时/连接拒绝/5xx。
        const pageOk = pageCode === "200" || pageCode === "401"
        checks.push({
          name: "HTTPS 页面",
          ok: pageOk,
          detail: pageCode === "401" ? "HTTP 401 = 入口可达，登录围栏正常（无凭据探测被拦，预期行为）" : "HTTP " + pageCode,
        })
        const sl = await apiSessionList(cfg.dshPort)
        let slOk = false, slDetail = sl.err || ""
        if (!sl.err) {
          // 401 / 纯文本 "unauthorized"：官方 /api 网关要求浏览器会话凭据，
          // Host 侧直连拿不到凭据被拒——说明网关活着且围栏正常，判为可达。
          if (sl.status === 401 || /^unauthorized\s*$/i.test((sl.raw || "").trim())) {
            slOk = true
            slDetail = "网关可达，要求登录凭据（无凭据 401 属预期；远程浏览器实际访问会带 cookie）"
          } else {
            try {
