...
async function apply(ctx, rawConfig) {
  ...
  const cookieHeaderFor = async (authority) => {
    const credentials = ctx.get("credentials")
    const record = credentials === undefined ? undefined : await credentials.readRecord("client-connection/browser-session")
    return mintSessionCookieFromRecord(authority, record, Date.now(), 30).setCookie
  }
  // 安装门户页 = 自动登录（本响应 Set-Cookie）+ 一键安装按钮；已装 APP（standalone）
  // 打开时页面脚本瞬间 location.replace('/') 进控制台，同时静默续期 cookie。
  const sendPortal = async (res, authority) => {
    res.writeHead(200, {
      "content-type": "text/html; charset=utf-8",
      "cache-control": "no-store",
      "referrer-policy": "no-referrer",
      "set-cookie": await cookieHeaderFor(authority),
    })
    res.end(renderPairSuccess())
  }
  const canAutoLogin = (req, authority) =>
    authority !== undefined && (tailscaleLoginOf(req) !== undefined || isLoopbackAuthority(authority))
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // 有配对码：兑换 → 成功落安装门户页（一键安装按钮），失败回输入页带提示
      if (code !== "") {
        const verdict = pairing.redeem(code, Date.now())
        if (verdict.ok && authority !== undefined) {
          await sendPortal(res, authority)
          return
        }
        // 码失效/过期：tailnet 身份仍在 → 直接进门户（链接对自家设备永远打得开）
        if (canAutoLogin(req, authority)) {
          await sendPortal(res, authority)
          return
        }
        res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" })
        res.end(renderPairPage({ error: authority === undefined
          ? "请求缺少 Host，无法签发会话"
          : verdict.reason === "expired" ? "配对码已过期（10 分钟），请在 Mac 面板重新生成"
          : "配对码无效或已被使用，请在 Mac 面板重新生成" }))
        return
      }
      // 无码：tailnet 身份或本机回环 → 自动登录 + 安装门户；否则输入页
      if (canAutoLogin(req, authority)) {
        await sendPortal(res, authority)
        return
      }
