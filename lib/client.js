  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      const copyPair = () => {
        ...
        setNotice("已复制配对链接，发送到需要登录的设备，用浏览器打开即可自动登录（10 分钟内有效，可多台设备同时使用）。")
      }
      // 本机一键安装（Chrome/Edge 原生安装对话框；iOS 无此 API，走 Safari 添加主屏幕）
      const standaloneMode = typeof matchMedia !== "undefined" && matchMedia("(display-mode: standalone)").matches
      const installApp = async () => {
        const p = window.__dshPwaInstall
        if (!p || !p.deferred) {
          setNotice("当前浏览器暂未提供一键安装：Chrome/Edge 打开本面板稍候再试；iPhone 用 Safari 分享→添加到主屏幕。")
          return
        }
        p.deferred.prompt()
        const r = await p.deferred.userChoice
        p.deferred = null
        setNotice(r.outcome === "accepted" ? "✓ 已安装到本机。可在桌面 / 启动台 / 主屏幕打开（独立窗口）。" : "已取消安装。")
      }
      ...
      return h("div", { className: "tspc" },
        ...
        !isRemotePage ? card("⑧ 设备配对（重启后免 token 重新登录）",
          h("div", { className: "tspc-row" },
            ...
            !standaloneMode ? btn("installApp", "安装桌面 APP", installApp) : null,
          ),
          ...
          h("div", { className: "tspc-note" }, "链接/二维码发给设备，打开即登录（10 分钟内有效，单次，30 天免登）。设备可长期书签 /tsctl/pair，过期后输新码即可；登录后按页内指引「添加到主屏幕」。仅本机面板可生成。手机端在 Tailscale 应用开启 Connect on Demand（iOS）或常驻 VPN（Android）后，连 Tailscale 都不用手动开。"),
        ) : null,
