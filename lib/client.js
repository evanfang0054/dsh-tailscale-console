...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      // 设备配对：本机面板生成一次性链接，远程设备打开即自动登录（免翻终端找 token）
      const pair = async () => {
        setBusy("pair"); setLog(""); setNotice(null)
        const res = await call("pairingCode")
        setBusy("")
        if (res.error) { setNotice("执行失败: " + res.error); return }
        setPairUrl((tsUrl || localUrl) + "/tsctl/pair?c=" + res.code)
      }
      const copyPair = () => {
        try { navigator.clipboard.writeText(pairUrl) } catch { /* 剪贴板不可用时用户可手选复制 */ }
      ...
      return h("div", { className: "tspc" },
        ...
        !isRemotePage ? card("⑧ 设备配对（重启后免 token 重新登录）",
          h("div", { className: "tspc-row" },
            ...
            pairUrl ? btn("copyPair", "复制链接", copyPair) : null,
          ),
          pairUrl ? h("div", { className: "tspc-row" }, ro(pairUrl)) : null,
          h("div", { className: "tspc-note" }, "每次重启 dsh 后所有设备的登录态都会失效。点上面按钮生成一次性链接（10 分钟有效），在手机/其他设备的浏览器打开即自动完成登录（30 天有效），无需再去终端找 token。仅本机面板可生成。"),
        ) : null,
